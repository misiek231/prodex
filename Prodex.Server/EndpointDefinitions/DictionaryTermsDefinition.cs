...
public class DictionaryTermsDefinition : IEndpointDefinition
{
    ...
    public void DefineEndpoints(RouteGroupBuilder group)
    {
        ...
        group.MapGet("{id}", async (IMediator mediator, ClaimsPrincipal user, [FromRoute] long id) => await mediator.Send(new SimpleGetDetails.Request<DictionaryTerm, FormModel>(id, user.Id())))
            .RequireAuthorization();
